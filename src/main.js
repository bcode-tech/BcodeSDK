...
export default class PablockSDK {
    ...
    async sendToken(contractAddress, spender, value, deadline, config) {
        const customERC20 = new ethers.Contract(contractAddress, abi, 
        ...
        "http://127.0.0.1:8082/sendPermit", { tx, contractAddress, address: this.wallet.address }, {
            headers: {
    ...
    async requestToken(contractAddress, to, amount) {
        let { status, data } = await axios.post(`${config[`ENDPOINT_${this.env}`]}/mintToken`, { contractAddress, to, amount }, {
            headers: {
                Authorization: `Bearer ${this.authToken}`,
            },
        });
